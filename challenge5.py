...
import requests
#module that allow us to work with dates and perform the
# functionalities that will aid us in completing this challenge.
from datetime import datetime, timedelta 
#module that allows us to tranform a string into an object that represents a date.
import dateutil.parser

"""
Name: Dario Molina
Date: 9/4/16
Description: Make a request to Code2040 API, retrieve a dictionary where one key is "datestamp" with a value that contains 
a string and it is a date formated in IOS 8601.  The second key is "interval" which is an integer representing seconds.
The goal of the program is to add the the seconds into the date that is in format IOS 8601, and send the new date back
 to the API for validation.  Make sure the new date we send to the API is in format IOS 8601 as well.
"""
...
#print the contents retrieved from our request
print r.text
...
date = json_dictionary['datestamp']#storing the datestamp value into date.  Value is of type string.
interval = json_dictionary['interval']#storing the amount of seconds into interval. Value is of type int.
...
Since the description says we need to send a date in the same format as the one we received from our request, 
I did a substring from 0-19 from newDateIOS and stored that new string into finalDate. The reason I chose from 0-19,
 is because we want to store the contents of our newDateIOS, up to but not including the '+'.  So we can then concatenate a 'Z' 
and finalDate could finally be in the right format that satisfies one of our objectives.

I had though about using a for loop that goes through each index and stores the elements of each index in finalDate
 up to it reached the index containing the '+'. But I though about the Big-O and using a for loop it would make our
 program to have O(n), but using a substring it would make our Big-O to be constant O(1) which is better.
"""
...
#printing the contents of our request t, informing us if we passed the challenge.
print t.text
